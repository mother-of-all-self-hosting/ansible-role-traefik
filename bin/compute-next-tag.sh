...
#
# Tags look like `v<Traefik version>-<release>`:
#
# - if defaults/main.yml points at a Traefik version that has never been
#   released, the release counter restarts at 0 (`v3.7.12-0`)
# - otherwise the counter is incremented (`v3.7.12-1`), but only if something
...
version="$(sed -nE 's|^traefik_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Traefik version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v3.1.0`),
